...
// 1. Get all accounts for an organization
...
// 2. Update user's current account choice
export async function updateCurrentAccount(userId, meliUserId) {
  const { data, error } = await supabase
    .from('organization_users')
    .update({ current_meli_account_id: meliUserId })
    .eq('user_id', userId);
...
// 3. Update organization default account (admin only)
export async function updateDefaultAccount(meliUserId) {
  const {organization_id} = await getUserInfo(supabase);
  const { data, error } = await supabase
    .from('organizations')
    .update({ default_meli_account_id: meliUserId })
...
export async function refreshMeliTokens(refreshToken) {
  try {
    ...
    // Store new tokens
    const rr = await storeMeliTokens({ tokens });
    console.log('storeMeliTokens result:', rr);
