// lib/meliTokens.js - Updated to use centralized Supabase client
import { createClient, getUserOrganization, getAuthenticatedUser } from '@/lib/supabase/server'
...
export async function storeMeliTokens({ tokens, is_default = false }) {
  try {
    ...
    console.error('Error in storeMeliTokens:', error)
    throw new Error(`Failed to store MeLi tokens: ${error.message}`)
  }  
}
...
export async function getMeliTokens() {
  const supabase = await createClient()
  const meli_user_id = await getCurrentMeliUserId(supabase)
  const organization_id = await getUserOrganization(supabase)
 ...
 */
export async function getCurrentMeliUserId(supabase = null) {
  const client = supabase || await createClient()
  const user = await getAuthenticatedUser(client)
  const organization_id = await getUserOrganization(client)
