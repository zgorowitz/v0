...
export async function getAllFilterItems() {
  ...
  let query = supabase
    .from('filter_items')
    .select('item_id, title, thumbnail, tags');
...
export async function totalSalesDaily(startDate, endDate, period = 'day', itemIds = null) {
  const organizationId = await getCurrentUserOrganizationId();
  ...
  const { data, error } = await supabase.rpc('get_total_sales_daily', params);
