...
export class QRBarcodeScanner {
  ...
  END:VCARD`
    ...
    extractResultString(parsedResult) {
      ...
      // Priority: shipment ID > original content
      
      if (isShipmentRelated(parsedResult.parsedData)) {
        return extractShipmentId(parsedResult.parsedData)
      }
      
      // For non-shipment QR codes, return original content
  ...
  export function parseUniversalContent(content) {
    ...
    const shipmentUrlMatch = content.match(/\/shipment\/(\w+)/i)
    if (shipmentUrlMatch) {
      return { 
        type: "shipment_url", 
        value: shipmentUrlMatch[1], 
        originalUrl: content,
        display: "Shipment Link"
      }
    }
  ...
  export function extractShipmentId(parsedContent) {
    ...
    if (parsedContent.type === 'shipment_url') {
      return parsedContent.value // Already extracted shipment ID
    }
