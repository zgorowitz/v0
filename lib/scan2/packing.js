...
// Get shipment data from function with fallback to API
export async function getShipmentData(shipmentId) {
  try {
    // Convert shipmentId to number for the RPC call
    ...
    const { data: viewData, error: viewError } = await supabase
      .rpc('get_shipment_packing_data', { p_shipment_id: shipmentIdNumber });

    if (!viewError && viewData && viewData.length > 0) {
      console.log('Shipment data retrieved from database function');
    ...
    // Fallback to API if not found in function
    console.log('Falling back to API for shipment data');
